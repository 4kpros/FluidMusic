...
import com.prosabdev.common.extensions.asAlbumArtContentUri
import com.prosabdev.common.media.FluidMusicNotificationManager
...
internal class PersistentStorage private constructor(private val ctx: Context) {
    ...
    class Player {
        companion object {
            ...
            suspend fun saveRecentSong(ctx: Context, description: MediaDescriptionCompat, position: Long) {
                withContext(Dispatchers.IO) {
                    val localIconUri = Glide.with(ctx).asFile().load(description.iconUri)
                        .submit(
                            FluidMusicNotificationManager.NOTIFICATION_LARGE_ICON_SIZE,
                            FluidMusicNotificationManager.NOTIFICATION_LARGE_ICON_SIZE
                        ).get()
                    ...
                    editor.putString(RECENT_SONG_ICON_URI_KEY, localIconUri.toString())
            ...
            fun loadRecentSong(): MediaBrowserCompat.MediaItem? {
                ...
                val extras = Bundle().also {
                    ...
                    it.putLong(MusicService.MEDIA_DESCRIPTION_EXTRAS_START_PLAYBACK_POSITION_MS, position)
                }
            ...
            //Repeat
            fun loadRepeat(): Int {
                return loadIntValue(PLAYER_REPEAT, PlaybackStateCompat.REPEAT_MODE_NONE)
            }
            fun saveRepeat(value: Int?) {
                saveIntValue(value ?: PlaybackStateCompat.REPEAT_MODE_NONE, PLAYER_REPEAT)
            }
            //Shuffle
            fun loadShuffle(): Int {
                return loadIntValue(PLAYER_SHUFFLE, PlaybackStateCompat.SHUFFLE_MODE_NONE)
            }
            fun saveShuffle(value: Int?) {
                saveIntValue(value ?: PlaybackStateCompat.SHUFFLE_MODE_NONE, PLAYER_SHUFFLE)
            }
            //Queue list source
