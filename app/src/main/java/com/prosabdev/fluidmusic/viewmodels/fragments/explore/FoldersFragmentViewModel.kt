...
class FoldersFragmentViewModel(app: Application) : GenericListenDataViewModel(app) {
    ...
    fun listenAllData(viewModel: FolderItemViewModel, lifecycleOwner: LifecycleOwner){
        MainScope().launch {
    ...
    suspend fun requestDataDirectlyFromDatabase(viewModel: FolderItemViewModel){
        mMutableDataList.value = viewModel.getAllDirectly(getSortBy().value ?: FolderItem.DEFAULT_INDEX)
    }
}
