package com.prosabdev.fluidmusic.utils

abstract class ConstantValues {

    companion object {

        const val TAG = "Fluid Music"
        const val PACKAGE_NAME = "com.prosabdev.fluidmusic"

        const val ORGANIZE_SMALL_LIST : Int = 1
        const val ORGANIZE_LIST : Int = 2
        const val ORGANIZE_LARGE_LIST : Int = 3
        const val ORGANIZE_SMALL_GRID : Int = 4
        const val ORGANIZE_GRID : Int = 5
        const val ORGANIZE_LARGE_GRID : Int = 6

        const val SHARED_PREFERENCES_BROADCAST = "${PACKAGE_NAME}.SHARED_PREFERENCES_BROADCAST"

        const val SHARED_PREFERENCES_REPEAT = "${PACKAGE_NAME}.SHARED_PREFERENCES_REPEAT"
        const val SHARED_PREFERENCES_SHUFFLE = "${PACKAGE_NAME}.SHARED_PREFERENCES_SHUFFLE"

        const val SHARED_PREFERENCES_SLEEP_TIMER = "${PACKAGE_NAME}.SHARED_PREFERENCES_SLEEP_TIMER"
        const val SHARED_PREFERENCES_CURRENT_PLAYING_SONG = "${PACKAGE_NAME}.SHARED_PREFERENCES_CURRENT_PLAYING_SONG"
        const val SHARED_PREFERENCES_PLAYING_PROGRESS_VALUE= "${PACKAGE_NAME}.SHARED_PREFERENCES_PLAYING_PROGRESS_VALUE"
        const val SHARED_PREFERENCES_QUEUE_LIST_SOURCE = "${PACKAGE_NAME}.SHARED_PREFERENCES_QUEUE_LIST_SOURCE"
        const val SHARED_PREFERENCES_QUEUE_LIST_SOURCE_VALUE = "${PACKAGE_NAME}.SHARED_PREFERENCES_QUEUE_LIST_SOURCE_VALUE"

        const val BUNDLE_CURRENT_SONG_META_DATA = "${PACKAGE_NAME}.BUNDLE_CURRENT_SONG_META_DATA"
        const val BUNDLE_QUEUE_LIST = "${PACKAGE_NAME}.BUNDLE_QUEUE_LIST"
        const val BUNDLE_SOURCE_FROM = "${PACKAGE_NAME}.BUNDLE_SOURCE_FROM"
        const val BUNDLE_SOURCE_FROM_VALUE = "${PACKAGE_NAME}.BUNDLE_SOURCE_FROM_VALUE"
        const val BUNDLE_SHUFFLE_VALUE = "${PACKAGE_NAME}.BUNDLE_SHUFFLE_VALUE"
        const val BUNDLE_REPEAT_VALUE = "${PACKAGE_NAME}.BUNDLE_REPEAT_VALUE"
        const val BUNDLE_CURRENT_SONG_ID = "${PACKAGE_NAME}.BUNDLE_CURRENT_SONG_ID"

        //Permissions
        const val BLUETOOTH_PERMISSION_CODE = 3
        const val AUDIO_RECORD_PERMISSION_CODE = 7
        const val STORAGE_PERMISSION_CODE = 11

        //foreground service
        const val SERVICE_DIED = 10
        const val FADE_UP = 11
        const val FADE_DOWN = 12
        const val FOCUS_CHANGE = 13
        const val GO_TO_NEXT_TRACK = 22

        // Actions for notifications
        const val TOGGLE_PLAY_PAUSE_ACTION = "${PACKAGE_NAME}.TOGGLE_PLAY_PAUSE_ACTION"
        const val PLAY_ACTION = "${PACKAGE_NAME}.ACTION_PLAY"
        const val PAUSE_ACTION = "${PACKAGE_NAME}.ACTION_PAUSE"
        const val STOP_ACTION = "${PACKAGE_NAME}.ACTION_STOP"
        const val NEXT_ACTION = "${PACKAGE_NAME}.ACTION_NEXT"
        const val PREV_ACTION = "${PACKAGE_NAME}.ACTION_PREV"
        const val FAVORITE_ACTION = "${PACKAGE_NAME}.FAVORITE_ACTION"
        const val REPEAT_ACTION = "${PACKAGE_NAME}.REPEAT_ACTION"
        const val CLOSE_ACTION = "${PACKAGE_NAME}.CLOSE_ACTION"
        const val SEEK_TO_ACTION = "${PACKAGE_NAME}.SEEK_TO_ACTION"

        const val MEDIA_SESSION_TAG = "${PACKAGE_NAME}.MEDIA_SESSION_TAG"

        //Notification mode
        const val HANDLER_THREAD = "${PACKAGE_NAME}.HANDLER_THREAD"
        const val CHANNEL_ID = "${PACKAGE_NAME}.CHANNEL_ID"
        const val NOTIFICATION_ID = "${PACKAGE_NAME}.21323123"
        const val NOTIFICATION_REQUEST_CODE : Int = 21301
        const val NOTIFICATION_MODE_NON = 0
        const val NOTIFICATION_MODE_FOREGROUND = 1
        const val NOTIFICATION_MODE_BACKGROUND = 2

        //Music library flags
        const val ARGS_EXPLORE_MUSIC_LIBRARY = "${PACKAGE_NAME}.ARGS_EXPLORE_MUSIC_LIBRARY"
        const val ARGS_EXPLORE_MUSIC_LIBRARY_VALUE = "${PACKAGE_NAME}.ARGS_EXPLORE_MUSIC_LIBRARY_VALUE"
        const val EXPLORE_MUSIC_CONTENT_FOR = "${PACKAGE_NAME}.EXPLORE_MUSIC_CONTENT_FOR"
        const val EXPLORE_MUSIC_CONTENT_FOR_VALUE = "${PACKAGE_NAME}.EXPLORE_MUSIC_CONTENT_FOR_VALUE"
        const val EXPLORE_ALL_SONGS = "${PACKAGE_NAME}.EXPLORE_ALL_SONGS"
        const val EXPLORE_ALBUMS = "${PACKAGE_NAME}.EXPLORE_ALBUMS"
        const val EXPLORE_ALBUM_ARTISTS = "${PACKAGE_NAME}.EXPLORE_ALBUM_ARTISTS"
        const val EXPLORE_ARTISTS = "${PACKAGE_NAME}.EXPLORE_ARTISTS"
        const val EXPLORE_COMPOSERS = "${PACKAGE_NAME}.EXPLORE_COMPOSERS"
        const val EXPLORE_FOLDERS = "${PACKAGE_NAME}.EXPLORE_FOLDERS"
        const val EXPLORE_GENRES = "${PACKAGE_NAME}.EXPLORE_GENRES"
        const val EXPLORE_YEARS = "${PACKAGE_NAME}.EXPLORE_YEARS"

        //Media scanner flags
        const val MEDIA_SCANNER_WORKER_NAME = "${PACKAGE_NAME}.MEDIA_SCANNER_WORKER_NAME"
        const val MEDIA_SCANNER_WORKER_SCAN_METHOD = "${PACKAGE_NAME}.MEDIA_SCANNER_WORKER_SCAN_METHOD"
        const val MEDIA_SCANNER_WORKER_OUTPUT = "${PACKAGE_NAME}.MEDIA_SCANNER_WORKER_OUTPUT"
        const val MEDIA_SCANNER_WORKER_METHOD_CLEAR_ALL = "${PACKAGE_NAME}.MEDIA_SCANNER_WORKER_METHOD_CLEAR_ALL"
        const val MEDIA_SCANNER_WORKER_METHOD_ADD_NEW = "${PACKAGE_NAME}.MEDIA_SCANNER_WORKER_METHOD_ADD_NEW"

        //Database strings
        const val FLUID_MUSIC_DATABASE_NAME = "${PACKAGE_NAME}.main_database"

        const val SELECTABLE_LIST_ADAPTER_TYPE_SONG_ITEM = "${PACKAGE_NAME}.SELECTABLE_LIST_ADAPTER_TYPE_SONG_ITEM"
        const val SELECTABLE_LIST_ADAPTER_FOLDER_URI_TREE_ITEM = "${PACKAGE_NAME}.SELECTABLE_LIST_ADAPTER_FOLDER_URI_TREE_ITEM"
        const val SELECTABLE_LIST_ADAPTER_QUEUE_MUSIC_ITEM = "${PACKAGE_NAME}.SELECTABLE_LIST_ADAPTER_QUEUE_MUSIC_ITEM"
    }
}