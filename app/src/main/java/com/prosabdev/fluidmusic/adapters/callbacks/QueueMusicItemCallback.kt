...
class QueueMusicItemCallback(adapter : QueueMusicItemListAdapter) : ItemTouchHelper.Callback() {
    ...
    override fun onMove(
        ...
        if(!mDragStarted){
            ...
            Log.i(Constants.TAG, "From position : $mFromPosition = $mFromValue")
        }
        mToPosition = target.bindingAdapterPosition
        Log.i(Constants.TAG, "To position : $mToPosition = $mToValue")
    ...
    override fun clearView(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder) {
        ...
        if (viewHolder is QueueMusicItemListAdapter.QueueMusicItemHolder) {
            ...
            mDragStarted = false
            Log.i(Constants.TAG, "Finally : from = $mFromPosition = $mToValue and to = $mToPosition = $mFromValue")
            mDragStarted = false
